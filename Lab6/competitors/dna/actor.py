...
class SimpleRLV3(Creature):
    ...
    WEIGHTS_FILE = os.path.join(
        str(Path(__file__).resolve().parent), "dna2_top10.npy"
    )  # or actorB

    @classmethod
    def load_best_dna(cls):
        """
        Loads the #1 best DNA from the saved CMA-ES top-10 list.
        If missing, fallback to DEFAULT_DNA.
        """
        if os.path.exists(cls.WEIGHTS_FILE):
            data = np.load(cls.WEIGHTS_FILE, allow_pickle=True)
            if len(data) > 0:
                best_reward, best_dna = data[0]  # entry format: (score, DNA)
                print(f"[SimpleRLV3] Loaded trained DNA with score={best_reward}")
                return np.copy(best_dna)

        print("[SimpleRLV3] Using DEFAULT_DNA (no trained file found)")
    ...
    def __init__(self, dna=None):
        ...
        if dna is not None:
            ...
            # Load best evolved DNA automatically
    ...
    @override
    @classmethod
    def instance_count(cls):  # type: ignore
        return SimpleRLV3.__instance_count

    @override
    @classmethod
    def destroyed(cls):  # type: ignore
        SimpleRLV3.__instance_count -= 1
