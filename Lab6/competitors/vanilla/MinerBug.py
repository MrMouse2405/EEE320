...
class MinerBug(Creature):
    __instance_count = 0

    def __init__(self):
        super().__init__()
        MinerBug.__instance_count += 1
        self.cilia = None
    ...
    def instance_count(cls):
        return MinerBug.__instance_count
    ...
    def do_turn(self):
        if not (self.cilia and self.type_sensor):
            ...
            return
        for d in Direction:
            if self.type_sensor.sense(d) is Plant:
                self.cilia.move_in_direction(d)
                return
        for d in Direction:
            if self.type_sensor.sense(d) in (Soil, Plant):
                self.cilia.move_in_direction(d)
                return
