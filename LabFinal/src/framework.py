...
class Receiver:
    def __init__(self, connection, event_loop, listeners):
        self.connection, self.event_loop, self.listeners = (
            connection,
            event_loop,
            listeners,
        )
...
class Snapshot:
    def __init__(self, simulation):
        ...
        self.counts = [
            competitor.instance_count() for competitor in simulation.competitor_classes
        ]
...
class ScoreBoard(tk.Frame):
    ...
    def initialize(self, snapshot):
        ...
        for ix, competitor in enumerate(snapshot.competitor_classes):
            ...
            row = 3 + ix
            dot.create_oval(
                b, b, b + self.DOT_SIZE, b + self.DOT_SIZE, fill=COLOURS[ix], width=0
            )
            ...
            group = tk.Label(
                self, text=self.group_name(competitor), font=self.SMALL_FONT
            )
            group.grid(row=row, column=2, sticky=tk.W)
            score = tk.Label(
                self, text="{:8d}".format(snapshot.counts[ix]), font=self.FONT
            )
...
class ControlPanel(tk.Frame):
    def __init__(self, master, simulation, competitor_classes):
        ...
        self.choose_button = tk.Button(
            self, text="Choose competitors", command=self.choose
        )
        self.simulation_interval = tk.DoubleVar()
        self.simulation_interval.set(250)
        self.speed_slider = tk.Scale(
            self,
            variable=self.simulation_interval,
...
class CompetitorSelector(tk.Toplevel):
    def __init__(self, parent, competitor_classes, callback, win_x, win_y):
        ...
        for row, competitor in enumerate(competitor_classes):
            v = tk.IntVar()
            self.vars.append(v)
            tk.Checkbutton(self, variable=v, text=competitor.__name__).grid(
                sticky=tk.W, row=row, padx=10, columnspan=3
            )
        row += 1
        tk.Button(self, text="Cancel", command=self.destroy).grid(
            row=row, column=0, padx=10
        )
        tk.Button(self, text="All", command=self.everyone).grid(
            row=row, column=1, padx=10
        )
        tk.Button(self, text="None", command=self.no_one).grid(
            row=row, column=2, padx=10
        )
        tk.Button(self, text="Go", command=self.select_competitors).grid(
