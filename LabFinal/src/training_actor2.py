...
# =====================================================
#     DNA_CMA_ES_BUG_BLUE (ANTI-STAGNATION VERSION)
# =====================================================
...
class DNA_CMA_ES_BUG_BLUE(Creature):
    ...
    def f_attack(self, defender):
        ...
        if def_before > 0 and defender is not self:
            DNA_CMA_ES_BUG_BLUE.reward_score += 5  # Small bonus for any combat
        ...
        if result is self and def_before > 0:
            ...
            if def_before > 500:
                DNA_CMA_ES_BUG_BLUE.reward_score += self.R_AGGRESSION_BONUS * (
                    def_before / 500
                )

        # Suicide attack = bad (but not catastrophic)
        if result is defender and defender is not self:
            DNA_CMA_ES_BUG_BLUE.reward_score -= 30  # Reduced penalty
    ...
    def do_turn(self):
        ...
        if DNA_CMA_ES_BUG_BLUE.current_turn >= DNA_CMA_ES_BUG_BLUE.turn_limit:
            return  # trainer finalizes reward

        DNA_CMA_ES_BUG_BLUE.reward_score += self.R_SURVIVE

        if self.f_apparent_type() is DNA_CMA_ES_BUG_BLUE:
            DNA_CMA_ES_BUG_BLUE.reward_score += self.R_TERRITORY
        ...
        if self.f_location() == old_pos:
            self.idle_turns += 1
            DNA_CMA_ES_BUG_BLUE.reward_score += self.R_IDLE
            if self.idle_turns >= 3:  # ultra-aggressive penalty
                DNA_CMA_ES_BUG_BLUE.reward_score -= 50
        else:
            self.idle_turns = 0
    ...
    def maybe_poison_drop(self):
        ...
        DNA_CMA_ES_BUG_BLUE.reward_score += (
    ...
    def maybe_reproduce(self):
        ...
        if after > before:
            ...
            DNA_CMA_ES_BUG_BLUE.reward_score += self.R_USELESS_REPRO
    ...
    def enemy_distance(self):
        ...
        for d in Direction:
            ...
            if t not in (Soil, Plant, DNA_CMA_ES_BUG_BLUE):
    ...
    def act_move(self):
        ...
        for direction in Direction:
            ...
            if cell is Plant:
                score += d[10]
            elif cell not in (Soil, DNA_CMA_ES_BUG_BLUE):
        ...
        if new_dist is not None:
            if self.last_enemy_distance is None:
                ...
                if new_dist < self.last_enemy_distance:
                    DNA_CMA_ES_BUG_BLUE.reward_score += self.R_APPROACH_ENEMY
                elif new_dist > self.last_enemy_distance:
                    DNA_CMA_ES_BUG_BLUE.reward_score += self.R_RETREAT_ENEMY
